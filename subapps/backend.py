...
def build_and_export_postal_code_geometries(
    ...
    target_dir = (
        ...
        else get_shapefile_data_dir(shapefile_url)
    )
    output_path = (
        Path(export_path)
    ...
    log("Lade Referenzdaten...")
    at_shapefile = build_at_shapefile(
        shapefile_zip_url=shapefile_url,
        shapefile_layer=shapefile_layer,
        target_dir=target_dir,
    )
    log("Erzeuge PLZ-Geometrien...")
    ...
    log(f"Exportiere nach {output_path}...")
    export_postal_code_geometries(
